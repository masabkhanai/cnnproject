...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.convolution_block4 = nn.Sequential(
            nn.Conv2d(
                in_channels=10, out_channels=20, kernel_size=(3,3), padding=0, bias=True
            ),
            nn.ReLU(),
            nn.BatchNorm2d(20),
        )
        ...
        self.convolution_block6 = nn.Sequential(
            nn.Conv2d(
                in_channels=32, out_channels=10, kernel_size=(3,3), padding=0, bias=True
            ),
            nn.ReLU(),
            nn.BatchNorm2d(10)
        )
        ...
        self.convolution_block_out = nn.Sequential(
            nn.Conv2d(
                in_channels=16, out_channels=2, kernel_size=(1,1), padding=0, bias=True
            )
